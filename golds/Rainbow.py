...
class RainbowAgent:
    ...
    def train(self, num_frames: int, env: AmericanOptionEnv, observation_grid: pd.DataFrame, cfg, save_freq: int, plotting_interval: int, save_path: str):
        """Train the agent."""
        self.is_test = False
        ...
        for frame_idx in range(1, num_frames + 1):
            ...
            if frame_idx % save_freq == 0:
                ...
                for _t in sorted(self.obs_input['normalized_time'].unique()):
                    ...
                    obs = np. hstack([self.obs_input.loc[self.obs_input['normalized_time'] == _t, ['price', 'all_1', 'call_price', 'normalized_time',]].values, prev_pos[:, np.newaxis], np.zeros((self.neposides, 2))])
                    pred = np.zeros(len(obs))
                    for j in range(len(obs)):
                        pred[j] = self.select_action(obs[j]) + self.cfg['action_min']
                    prev_pos = pred
                    preds.append(pred)
    ...
    def calculate_reward_vectorize(self, res, cfg):
        ...
        res['reward'] =  (-0.5) * res['delta_wealth'] ** 2 * cfg['reward_kappa'] + res['delta_wealth'] # this is reward function
        return res
